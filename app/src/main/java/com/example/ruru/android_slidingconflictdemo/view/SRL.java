...
package com.example.ruru.android_slidingconflictdemo.view;
 ...
 * </p>
 */
public class SRL extends ViewGroup implements NestedScrollingParent,
    ...
    private boolean mIsBeingDragged;//是否被拽
    private int mActivePointerId = INVALID_POINTER;
    // Whether this item is scaled up rather than clipped
    boolean mScale;//是否被缩放

    // Target is returning to its start offset because it was cancelled or a
    // refresh was triggered.
    private boolean mReturningToStart;  //刷新被取消后回到偏移前的位置
    private final DecelerateInterpolator mDecelerateInterpolator;//减速插值器。View动画，属性动画，Scroller都有插值器。
    private static final int[] LAYOUT_ATTRS = new int[]{
            android.R.attr.enabled
    };

    CircleImageView mCircleView;//圆形
    ...
    CircularProgressDrawable mProgress;//圆形进度条
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            if (mRefreshing) {//正在刷新
                ...
                mProgress.setAlpha(MAX_ALPHA);//最大完全不透明，最小完全透明
                mProgress.start();
                if (mNotify) {//通知正在刷新
                    if (mListener != null) {
                        mListener.onRefresh();
    ...
    void reset() {
        ...
        setColorViewAlpha(MAX_ALPHA);//progress和circleView完全透明
    ...
    public void setEnabled(boolean enabled) {
        super.setEnabled(enabled);//为false时重置初始化状态
        if (!enabled) {
            reset();
        }
    }
     ...
     *
     *              刷新开始和休息都在刷新内容的上方。
     *              如果进度条Z轴上方没有view了就设置为true。设置为true意味着显示被缩放，按比例放大而不是被剪切。
     *              start：progress出现view顶部px的偏移
     *              end：progress休息后view顶部px的偏移
     */
    ...
    public void setProgressViewEndTarget(boolean scale, int end) {
        mSpinnerOffsetEnd = end;
        mScale = scale;
        mCircleView.invalidate();//重新绘制整个View。如果View可见，Canvas的onDraw方法将来肯定会调用。必须在UI线程中调用，非UI线程中用postInvalidate。
    }
    ...
    public SwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();//滑动最小距离

        mMediumAnimationDuration = getResources().getInteger(
                android.R.integer.config_mediumAnimTime);//中等动画间隔

        setWillNotDraw(false);
        mDecelerateInterpolator = new DecelerateInterpolator(DECELERATE_INTERPOLATION_FACTOR);//动画减速插值器
    ...
    public void setRefreshing(boolean refreshing) {
        if (refreshing && mRefreshing != refreshing) {
            ...
            mNotify = false;
            startScaleUpAnimation(mRefreshListener);//开始按比例放大动画，刷新控件是按比例放大的？？
        } else {
    ...
    void startScaleDownAnimation(Animation.AnimationListener listener) {
        mScaleDownAnimation = new Animation() {
            @Override
            public void applyTransformation(float interpolatedTime, Transformation t) {//自定义动画
    ...
    /**
     * 布局。ViewGroup确定子元素的位置。View确定自己的位置。
     */
    @Override
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        final int width = getMeasuredWidth();//测量宽度
        final int height = getMeasuredHeight();//测量高度
        if (getChildCount() == 0) {
            return;
        }
        if (mTarget == null) {
        ...
        child.layout(childLeft, childTop, childLeft + childWidth, childTop + childHeight);//子View进行布局
